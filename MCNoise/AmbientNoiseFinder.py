...
class AmbientNoiseFinder(object):
    def __init__(
        ...
        ## Data columns added to cells addata:
        - umi_depth - the total number of umis in the cell, added to obs.
        - batch - the batch name of each cell, added to obs.
        - cells_cluster - the id of the cluster matching the metacell this cell belong to, added to obs.
        - umi_depth_bin - the bin id matching this umi_depth, added to obs.
        - genes_cluster - the id of the gene cluster, added to var. Will contain -1 if no candiadte gene.

        ## Data columns added to metacell addata:
        ...
        if len(metacells_to_ignore):
            self.metacells_adata = mc.ut.slice(
                self.metacells_adata,
        ...
        if genes_clusters.empty:
            ...
            self.logger.info("Using provided gene clusters")
            self.small_genes_clusters = genes_clusters.value_counts()[
                genes_clusters.value_counts() < minimum_genes_in_cluster
            ].index
        ...
        if metacells_clusters.empty:
            self.logger.info(
                "No metacell clusters provided, will create %s clusters"
                % number_of_metacells_clusters
            )
            (
                metacells_clusters,
            ...
            self.logger.info("Using provided metacells clusters")
            self.small_metacells_clusters = metacells_clusters.value_counts().index[
                metacells_clusters.value_counts() < minimum_metacells_in_cluster
            ]
    ...
    def _get_candidates_genes(
        ...
        self.logger.debug(
            "Getting genes candidatesfor clustering, minimum log fold of %s"
        ...
        self.logger.debug(
            "Found %s candidates genes to cluster" % len(candidates_genes)
        )
    ...
    def _get_genes_clusters(
        ...
        if len(small_genes_clusters):
            self.logger.info(
                "%s genes clusters have low number of genes (< %d). Pass use_small_genes_clusters to AmbientNoiseEstimator if you wish to use them"
                % (len(small_genes_clusters), minimum_genes_in_cluster)
            )

        for gene_cluster_id in small_genes_clusters:
            self.logger.info(
                "Small gene cluster id: %s, genes: %s"
                % (
                    gene_cluster_id,
                    ", ".join(
                        clusters_series[clusters_series == gene_cluster_id].index
                    ),
                )
            )
    ...
    def _get_metacells_clusters(
        ...
        clusters_series = pd.Series(
            kmeans.labels_.astype(int), index=range(len(self.metacells_adata.obs.index))
        )
        ...
        if len(small_metacells_clusters):
            self.logger.info(
                "%s metacell clusters have low number of metacells (< %d). Pass use_small_metacell_clusters to AmbientNoiseEstimator if you wish to use them"
                % (len(small_metacells_clusters), minimum_metacells_in_cluster)
            )

        for metacell_cluster_id in small_metacells_clusters:
            self.logger.info(
                "Small metacell cluster id: %s, metacells: %s"
                % (
                    metacell_cluster_id,
                    ", ".join(
                        clusters_series[
                            clusters_series == metacell_cluster_id
                        ].index.astype(str)
                    ),
                )
            )
    ...
    def plot_expression_diff_between_clusters(
        ...
        with sb.plotting_context(rc={"font.size": 30}):
            ...
            ax.set_xlabel("Genes clusters")
            ax.set_ylabel("Metacell clusters")
    ...
    def plot_genes_mc_clusters(self) -> None:
        ...
        for i, gene_cluster in enumerate(genes_clusters):
            if gene_cluster == -1:
                continue
            genes = self.metacells_adata.var[
                self.metacells_adata.var.genes_cluster == gene_cluster
            ].index
